...
func (dm *DownloadManager) requestPieces(c *client.Client, session *peerSession) {
	for {
		select {
		case <-dm.downloadCompleteChan:
			return

		case piece, open := <-dm.pieceQueue:
			if !open {
				log.Println("Queue is closed, exiting piece requestor")
				return
			}
			...
			for begin := 0; begin < piece.length; begin += MAX_BLOCK_SIZE {
				...
				if err := c.SendRequest(piece.index, begin, length); err != nil {
					log.Printf("[ Block req err: %s ] [ index: %d, begin: %d, length: %d ]", err.Error(), piece.index, begin, length)
					dm.pieceQueue <- piece
					continue
				}
			}
